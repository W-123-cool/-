...
ELEVATOR_POSITIONS = {
    "1F": {
        ...
        "initial_pose": {"x": -0.254, "y": 0.551, "yaw": 0.203},
    },
    "2F": {
        "entry": {"x": 6.17, "y": -3.17, "yaw": 0.134},
        "initial_pose": {"x": 6.17, "y": -3.17, "yaw": 0.134},
    },
}
...
ROOM_LOCATIONS = {
    ...
    "102": {"floor": "1F", "x": 1.8, "y": -6.03, "yaw": 0.0023},
    "103": {"floor": "1F", "x": 5.02, "y": 0.452, "yaw": 0.00124},
    "104": {"floor": "1F", "x": -2.79, "y": 3.86, "yaw": 0.0},
    "200": {"floor": "2F", "x": 6.17, "y": -3.17, "yaw": 0.134},
    "201": {"floor": "2F", "x": 1.8, "y": -6.03, "yaw": 0.0023},
    "202": {"floor": "2F", "x": 5.02, "y": 0.452, "yaw": 0.00124},
...
class SmartBuildingNavigator(Node):
    ...
    def __init__(self) -> None:
        ...
        # CONNACK 由 loop 线程处理：connect() 返回后可能尚未连上，status 发布与 subscribe 须等 on_connect
    ...
    def on_mqtt_connect(self, client, userdata, flags, rc, properties=None):
        ...
        self.get_logger().info(
            ...
            f"subscribed request={self._topic_request}, {TOPIC_ELEV_RESP}, {TOPIC_NAV_ROOM_LEGACY}"
        )
        try:
    ...
    def request_elevator(self) -> None:
        ...
        # 发 MQTT 后取消 Nav2，避免电梯阶段位姿异常导致继续规划；取消产生的中止结果在 _on_nav_done 中忽略。
        self.cancel_nav()
    ...
    def _on_nav_done(self, success: bool) -> None:
        ...
        # legacy_xf：robot/nav_room 跨层时与 deliver_room_xf 一样先到电梯再 MQTT 呼梯
        if ctx in ("dev_elevator", "deliver_room_xf", "return_home_xf", "legacy_xf"):
            self.request_elevator()
            return
