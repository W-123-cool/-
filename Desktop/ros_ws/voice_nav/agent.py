...
from .tour_coordinator import TourCoordinator
...
class VoiceNavAgent:
    def __init__(self) -> None:
        ...
        self.tour_coord: Optional[TourCoordinator] = None
        if os.environ.get("VOICE_TOUR_COORDINATOR", "1").strip().lower() not in (
            "0",
            "false",
            "no",
        ):
            try:
                self.tour_coord = TourCoordinator(self.nav)
            except Exception as exc:
                print(f"[tour] coordinator disabled: {exc}", flush=True)
    ...
    def mission_active(self) -> bool:
        ...
        if self.nav_session.mission_active():
            return True
        if self.tour_coord and self.tour_coord.use_api():
            self.tour_coord.refresh()
            if self.tour_coord._phase in ("navigating", "returning"):
                return True
    ...
    def handle_wake_stop_mission(self) -> None:
        """导览途中：唤醒词截停 → 取消当前导航，原地待机（不返航）。"""
        self._processing = True
        try:
            self.session.clear_pending()
            if self.tour_coord and self.tour_coord.use_api():
                ok, msg = self.tour_coord.on_wake_stop()
                print(f"[wake_stop] api {ok}: {msg}", flush=True)
                self.mark_nav_ended(MissionPhase.ARRIVED)
            else:
    ...
    def _handle_cancel_navigation(self) -> bool:
        ...
        tts.stop()
        if self.tour_coord:
            ok, msg = self.tour_coord.cancel_navigation()
            print(f"[\u5bfc\u822a\u53d6\u6d88] {msg}", flush=True)
        else:
            self.mark_nav_ended(MissionPhase.CANCELLED)
    ...
    def _start_navigation(self, room_id: str, *, floor_hint: str = "") -> bool:
        ...
        if room is None:
            ...
            return False
        if self.tour_coord:
            ok, msg, _tid = self.tour_coord.start_navigation(room_id)
    ...
    def _execute_room_intent(
        ...
        *,
        ...
        if intent in ("qa", "qa_then_navigate", "navigate", "unknown"):
            ...
            if self.tour_coord:
                self.tour_coord.report_qa_touch()
