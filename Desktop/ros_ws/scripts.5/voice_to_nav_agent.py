...
from voice_nav.input_trigger import make_input_trigger
...
def main() -> None:
    ...
    os.environ.setdefault("VOICE_INPUT_MODE", "enter")
    ...
    print("  nav MQTT: robot/nav_room  cancel: robot/nav_cancel", flush=True)
    ...
    try:
        while True:
            ...
            if voice_state == STATE_KWS_BOOT:
                if not _wait_kws_wake(wake_gate, capture):
                    break
                wake_gate.pause_after_wake()
            ...
            if not trigger.wait_trigger(hint="[输入] 按回车开始录音（q 退出）…"):
                break
