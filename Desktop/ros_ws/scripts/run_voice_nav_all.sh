...
echo ""
echo -e "${GREEN}终端1 — 语音导览/导航（UI 按键 / 唤醒词）${NC}"
echo -e "${CYAN}麦克风: ${AUDIO_DEV}${NC}"
export VOICE_INPUT_MODE="${VOICE_INPUT_MODE:-ui}"
export VOICE_TOUR_API_BASE="${VOICE_TOUR_API_BASE:-${COURIER_API_BASE:-http://127.0.0.1:8000}}"
export COURIER_API_BASE="${COURIER_API_BASE:-$VOICE_TOUR_API_BASE}"
echo -e "${CYAN}UI-PTT:   mode=${VOICE_INPUT_MODE} api=${VOICE_TOUR_API_BASE}${NC}"
ai_car_validate_tour_api_base || echo -e "${YELLOW}[warn] UI-PTT backend unreachable — fix scripts/onboard_api.env then restart voice${NC}"
echo -e "${CYAN}交互: 网页点语音 / 唤醒词 -> 再点开始 -> 说话 -> 再点结束${NC}"
